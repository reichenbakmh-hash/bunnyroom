...
export default function OnboardingPage({ onDone }: { onDone: () => void }) {
  ...
  async function handleCreate() {
    ...
    if (relError || !relationship) {
      setError(`Impossible de créer la relation. ${relError ? `(détail : ${relError.message})` : ''}`)
      setLoading(false)
      return
    }
    ...
    if (memberError) {
      setError(`Impossible de t'ajouter à la relation. (détail : ${memberError.message})`)
      setLoading(false)
      return
    }
  ...
  async function handleJoin() {
    ...
    if (joinError) {
      setError(`Impossible de rejoindre cette relation. (détail : ${joinError.message})`)
      setLoading(false)
      return
    }
